...
//#endif
	...
	//sp_item_dialog(); //////////////!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    //Gtk::Widget *itemw = Glib::wrap(sp_item_widget_new (this));
	this->sage_widget = Glib::wrap(sp_item_widget_new ());
	//contents->pack_start(*itemw, TRUE, TRUE, 0);
	contents->pack_start(*sage_widget, TRUE, TRUE, 0);
	
	this->apply_bttn.signal_clicked().connect(sigc::mem_fun(*this, &ObjectProperties::apply_button));
	...
	contents->pack_start(button_row, FALSE, FALSE, 0);
...
void ObjectProperties::HideWindow()
{
	signalResponse().emit(GTK_RESPONSE_CLOSE);
}
...
void ObjectProperties::apply_button()
{
	color_deletelastrow();
	clone_deletelastrow();
	isac_update_syntax(NULL, SP_WIDGET(sage_widget->gobj()));
	sp_item_widget_label_changed (NULL, SP_WIDGET(sage_widget->gobj()));
}
...
{
    //sp_signal_disconnect_by_data (INKSCAPE, dlg);	//HSH removed
    //wd.win = dlg = NULL;
    //wd.stop = 0;
}
